...
function formatNote(n) {
  return { ...n, is_favorite: toBool(n.is_favorite), is_pinned: toBool(n.is_pinned) };
}

router.get('/', async (req, res) => {
  try {
    const db = getDb();
    const { search, category, favorite, sort = 'updated_at', order = 'desc', limit = 100, offset = 0 } = req.query;
    ...
    res.json({ data: rows.map(formatNote), total: parseInt(count) });
...
router.post('/', async (req, res) => {
  try {
    const db = getDb();
    const { title, content, description, category, color } = req.body;
